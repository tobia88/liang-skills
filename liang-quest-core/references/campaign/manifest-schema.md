...
No `workflow` field. The planner-native pipeline has a single executor, so no workflow discriminator is needed. The `workflow` field belongs to the deprecated cartographer/tactician chain only.
...
## Deprecated Schema (Cartographer → Tactician → Executor)

> **DEPRECATED.** Retained for in-flight campaigns. Do not use for new work — use the canonical schema above.
