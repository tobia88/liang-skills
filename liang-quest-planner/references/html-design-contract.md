...
The planner assembles `plan.html` from three fixed layers rather than regenerating CSS every run. Body drafting is delegated to the body-drafter subagent (model resolved per SKILL.md Phase 2c: `project.yaml` `models.body_drafter` → `execution_by_difficulty.medium` → harness default, skipping steps the current harness cannot spawn; tier-alias harnesses resolve via `models.claude_mode`); CSS inlining and structural validation are owned by a deterministic script — no model re-types CSS in any standard path.
