...
The planner assembles `plan.html` from three fixed layers rather than regenerating CSS every run. The drafter's input is the campaign's `plan.md` and quest files, read from disk — the planner has already written them when the render runs. Body drafting is delegated to the body-drafter subagent (model resolved per SKILL.md Phase 2c: `project.yaml` `models.body_drafter` → `execution_by_difficulty.medium` → harness default, skipping steps the current harness cannot spawn; tier-alias harnesses resolve via `models.claude_mode`); CSS inlining and structural validation are owned by a deterministic script — no model re-types CSS in any standard path.
...
- Revisions:
  ...
  - v2.3 — **body-drafter model resolution made harness-aware.** A chain step that resolves to a model the current harness cannot spawn is treated as unresolved (continue down the chain), and tier-alias harnesses resolve through `models.claude_mode.body_drafter` → `claude_mode.medium` → the namespace's medium default instead. Root cause: a pi-side `execution_by_difficulty.medium` value resolved successfully but was unspawnable under a tier-alias harness, and the spec had no rule for "resolved but unspawnable," so the drafter silently inherited the session model — the most expensive possible outcome; the schema doc's old step-3 claim of a "sonnet-class" harness default was also wrong (no override means session-model inheritance). The planner now announces the resolved drafter model in one line before spawning. Schema updated in `liang-quest-core/references/project/project-yaml.md`: `claude_mode` gains additive-optional `body_drafter`, no schema bump.
