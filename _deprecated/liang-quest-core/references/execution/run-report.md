...
Required sections: `# Run Report`, `## Quest Results`, `## Decisions needed` when `totals.decisions_needed` > 0, `## Plan drift` when `totals.plan_drift` > 0, `## Deferred UAT`, `## Lessons`, `## Spend` when `usage_tracked` is true, and `## Shared Helpers` when applicable.
