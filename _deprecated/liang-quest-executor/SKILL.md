...
- **VC repair rounds:** a failed Tier 1 VC does not fail the quest at once. Up to `executor.max_vc_repair_rounds` (default 2) repair rounds turn the verifier's evidence into a fix, rebuild and re-test, and re-verify every Tier 1 VC (§7d).
- **Known-failure playbook:** before any step retry or repair round, the executor classifies the failed build/test logs with the shared playbook (`liang-quest-core/scripts/failure_playbook.py`). A false failure passes the step, a transient failure retries for free, a blocker fails the quest straight away (§7c).
...
Read `.liang/project.yaml`. If absent: bootstrap it through `liang-quest-core/references/project/project-yaml.md` § First-Run Interview; under `--no-confirm`, stop with exit code 2. If present: validate `schema_version`, `vcs`, `models.planning`, `models.execution_by_difficulty.{easy,medium,hard}`; read optional `executor.max_step_retries` (default 3), `executor.max_vc_repair_rounds` (default 2), `executor.child_timeout_seconds` (default 300), `executor.unattended` (default false; `true` implies `--no-confirm` unless `--confirm` was passed — resolve this **before** §1 runs), and in `--claude` mode `models.claude_mode.{easy,medium,hard,verify,planning}` (all optional; defaults per `liang-quest-core/references/project/project-yaml.md` § Model Routing Extensions).
...
**Playbook first.** Before counting a failure against the step, run `python <skills-root>/liang-quest-core/scripts/failure_playbook.py <the step's build/test logs>` and act on its `verdict`: `false_failure` → the step succeeded (every error line is a known-harmless one, e.g. the open editor's engine DLL lock); record the matched rule ids in the envelope and continue. `retry` → re-run the unchanged step once without spending a retry (at most twice per step). `blocker` → fail the quest with `failure_type: "blocker"` and the rule's note (a human must act; retrying cannot help). `hint` → pass the rule notes to the next attempt as a lesson. `unknown` / `clean` → the normal loop below.
...
Skip if the quest already failed in the step loop (proceed to §7e). Otherwise auto-classify each `## Victory Conditions` checkbox: mechanical patterns → **Tier 1** (verify inline; spawn a verify-child only when the check needs reasoning over file contents); judgmental → **Tier 2** (add to the deferred UAT queue — never verified inline). All Tier 1 pass → quest passes provisionally. Any Tier 1 fail → **VC repair round** (up to `executor.max_vc_repair_rounds`, default 2): a re-plan-child turns the failed VCs and the verifier's evidence into fix instructions, an execute-child applies them and re-runs the quest's own build/test commands, and every Tier 1 VC is verified again. Each round is recorded in `.run/<quest-id>/step-fix<r>.md`. Still failing after the last round → quest `failed`, lesson `failure_type: "vc_failed"`. A repair that had to change a test's expectations or a VC's meaning passes but is flagged `needs_review`. Pattern list, verify-child prompts, repair rules and aggregation: `references/vc-verification.md` — load when classifying.
...
3. Write `.run/<quest-id>/complete.yaml` with quest summary — including `vc_repair_rounds` (when > 0), `needs_review` (list of reasons, when flagged), `playbook` (matched rule ids, when any), and `failure_type` / `failed_vcs` on failure; the batch sweep reads these for its event stream and morning report — and the quest `usage` rollup — sum across every pinned session in `.run/<quest-id>/sessions/` (execute attempts including retries, re-plan, verify); write the same `total_tokens` / `cost_usd` to the quest's manifest `usage` field. When nothing was harvested, omit `usage` everywhere — never write zeros.
...
19. **Invent steps.** No "addendum" or "extra" steps outside the quest's `## Steps`. The one documented exception is a §7d VC repair round, which may only make an already-listed Tier 1 VC pass. Work the plan did not foresee is either a re-plan-child revision of an existing step (recorded in that step's envelope) or an entry in `## Decisions needed` for the next planning pass. A quest cannot pass on work its own step list never asked for.
