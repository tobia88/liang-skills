...
This document governs the optional `plan.html` render produced by `liang-quest-planner` when `planner.html` is true in `.liang/project.yaml`. `plan.md` and the quest files are the source; `plan.html` is a render of them, regenerated whole and never edited in place. With `planner.html` false nothing in this document applies. It defines the **quality floor every output must clear**, the **aesthetic catalog** the planner picks from, the **selection heuristics** for matching direction to campaign, the **delegation protocol** for the body-drafting subagent, the **CSS pitfall guardrails** to avoid common AI-generated layout bugs, and the **anti-patterns** that mark generic AI-slop output.

Read this document **before every render**, whether delegating or generating inline.
...
The planner assembles `plan.html` from three fixed layers rather than regenerating CSS every run. The drafter's input is the campaign's `plan.md` and quest files, read from disk — the planner has already written them when the render runs. Body drafting is delegated to the body-drafter subagent (model resolved per `liang-quest-core/references/project/project-yaml.md` § Model Routing Extensions); CSS inlining and structural validation are owned by a deterministic script — no model re-types CSS in any standard path.

1. **The body-drafter subagent generates ONLY the fixed-body HTML** per `references/templates/class-contract.md` — the variable per-campaign content (masthead, TOC, quest sections, notes, footer). It is briefed with **file paths, not transcribed content**: `plan.md` and every quest file in the campaign folder, plus the class contract itself. The Decision Summary comes from `plan.md`'s Decision Summary section, the quest content from its Quests section plus the quest files, and the visual recipe from `plan.md`'s Visual line (present only when Phase 2a planned a plan visual). The HTML structure and class names are identical regardless of which aesthetic direction is chosen. No theme needs bespoke markup.
...
The body generator — the body-drafter subagent, or the planner in fallback — emits **only the fixed semantic body** — no inline CSS beyond the whitelisted custom properties, no `<style>` block, no palette selection in the body markup. The body skeleton is defined in `references/templates/class-contract.md` and is identical across all aesthetic directions.

### 4.2 Delegation Scope

Body drafting goes to a general-purpose subagent on the resolved body-drafter model (configurable via `project.yaml` — see §4 intro). The delegation is transcription, never decision-making: quest decomposition, difficulty, visual recipe, and aesthetic direction are all decided by the planner and already written into `plan.md` + the quest files before the subagent is briefed. The subagent reads those files from disk and transcribes them; it invents nothing and re-decides nothing. CSS is never delegated to any model — `assemble_plan.py` owns it. If subagent spawning is unavailable, or the subagent fails validation twice, the planner drafts the body inline and still assembles via the script. Manual CSS inlining is a last resort reserved for environments without Python.
...
- Revisions:
  ...
  - v2.4 — **`plan.md` is the source; `plan.html` is an optional render.** The planner now writes a required `plan.md` dossier plus the quest files, and renders `plan.html` from them only when `planner.html` is true in `.liang/project.yaml` (`--html` / `--no-html` override per run; no default, no `auto`). The drafter's brief carries **file paths** instead of transcribed content — it reads `plan.md` and the quest files from disk, takes the Decision Summary from `plan.md`'s Decision Summary section and the visual recipe from its Visual line. **Edit-in-place is removed**: every render is a full regeneration from disk, including each Phase 3 turn that changed something. Nothing in this document applies when `planner.html` is false.
  - v2.3 — **body-drafter model resolution made harness-aware.** Unspawnable chain steps are now skipped and tier-alias harnesses get their own chain (rule text: `liang-quest-core/references/project/project-yaml.md` § Model Routing Extensions). Root cause: a pi-side `execution_by_difficulty.medium` value resolved successfully but was unspawnable under a tier-alias harness, and the spec had no rule for "resolved but unspawnable," so the drafter silently inherited the session model — the most expensive possible outcome; the schema doc's old step-3 claim of a mid-tier harness default was also wrong (no override means session-model inheritance). The planner now announces the resolved drafter model in one line before spawning. Schema updated in `liang-quest-core/references/project/project-yaml.md`: `claude_mode` gains additive-optional `body_drafter`, no schema bump.
