---
description: Audit/refine the liang-quest-* and liang-brainstorm-* skill families against the family criteria (C1-C9). audit = read-only report, refine = tiered apply, enhance = named behavioral change.
...
target glob (default: every family's `members_glob`).

At activation, read these from the liang-skills root. **Stop and report failure if
any read fails.**

- `_family-audit/criteria-common.md` — the invariants (C1–C9) in generic form, edit
  classes, the done bar, and the findings schema.
- `liang-<family>-core/references/family/criteria.md` for each targeted family — what
  the criteria mean for that family, plus the `family:` data block. Together with the
  common file, the sole source of what to check; never invent checks beyond them.
- `_family-audit/drift-ledger.md` and each targeted family's
  `references/family/drift-ledger.md` — intentional divergences. Anything matching a
  ledger entry is not a finding; cite the dv ID when skipping.

## Flow

1. **Preflight (zero model tokens).** Run
   `python _family-audit/preflight.py --targets "<glob>"`. It writes `findings.yaml`
...
2. **Judgment sweep.** For each criterion preflight flagged `judgment-needed`
   (C1 restatement-vs-orchestration, C3 delegation paths, C5 conventions, C9 topology wording), spawn one
   child per dimension — Claude harness: subagent; pi harness: `pi --print`
   sub-process. Build each child's input from `_family-audit/check-brief.yaml`
...
3. **Classify.** Tag every finding `mechanical` / `structural` / `behavioral` per
   `_family-audit/criteria-common.md` § Edit classes (C6). Dedup against the drift ledger; move suppressed findings to the
...
4. **Apply** (refine mode only). First re-take the tree snapshot and diff it
   ...
   - `behavioral` — refuse; report the finding with a note that it requires
     `enhance "<goal>"`. Under `enhance`, log every behavioral change as a new row
     in the family's `references/family/decisions.md` when it keeps one.
...
- Never add or remove drift-ledger entries — propose candidates as `open-question`
  findings instead.
- Never restate criteria into this command file — the criteria files are the source
  of truth.
